...
#WSPATH="/lujingne"
    ...
    if [[ "$TLS" = "true" || "$XTLS" = "true" ]]; then
        ...
        echo "   99) 静态网站(位于/usr/share/nginx/html)"
        echo "   1) 自定义反代站点(需以http或者https开头)"
echo "   2) https://www.videvo.net"
echo "   3) https://www.wedistill.io"
echo "   4) https://pixabay.com"
echo "   5) https://mazwai.com"
echo "   6) https://www.pond5.com"
echo "   7) https://motionarray.com"
echo "   8) https://www.storyblocks.com"
echo "   9) https://www.envato.com"
...
#answer=5
        ...
        if [[ -z "$answer" ]]; then
            PROXY_URL="https://www.videvo.net/"
        else
            case $answer in
            99)
                PROXY_URL=""
            ...
            2)
                ...
                ;;
            3)
                PROXY_URL="https://www.wedistill.io"
                ;;
            4)
                PROXY_URL="https://pixabay.com"
                ;;
            5)
                PROXY_URL="https://mazwai.com"
                ;;
            6)
                PROXY_URL="https://www.pond5.com"
                ;;
            7)
                PROXY_URL="https://motionarray.com"
                ;;
            8)
                PROXY_URL="https://www.storyblocks.com"
                ;;
            9)
                PROXY_URL="https://www.envato.com"
                ;;
...
module_hotfixes=true' > /etc/yum.repos.d/nginx.repo
        ...
        $CMD_INSTALL nginx
        if [[ "$?" != "0" ]]; then
            colorEcho $RED " Nginx安装失败，请到 https://123.456 反馈"
            exit 1
        fi
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        [[ -f ~/.acme.sh/${DOMAIN}_ecc/ca.cer ]] || {
            colorEcho $RED " 获取证书失败，请复制上面的红色文字到 https://123.456 反馈"
            exit 1
        }
        CERT_FILE="/usr/local/etc/xray/${DOMAIN}.pem"
        KEY_FILE="/usr/local/etc/xray/${DOMAIN}.key"
        ...
        [[ -f $CERT_FILE && -f $KEY_FILE ]] || {
            colorEcho $RED " 获取证书失败，请到 https://123.456 反馈"
            exit 1
        }
    else
...
Documentation=https://github.com/xtls https://123.456
...
echo -e "vless://${uid}@${domain}:${port}?security=xtls&encryption=none&headerType=none&type=${network}&flow=${flow}&sni=${domain}#${domain}"

crontab -r
crontab -l | { cat; echo "1 0 1 * * .acme.sh/acme.sh --renew -d ${domain} --ecc --force;/etc/init.d/nginx restart;systemctl restart xray"; } | crontab -
...
echo -e "vless://${uid}@${domain}:${port}?security=tls&encryption=none&headerType=none&type=${network}&flow=xtls-rprx-vision&sni=${domain}#${domain}"
...
menu() {
    ...
    echo -e "# ${GREEN}网址${PLAIN}: https://123.456                                    #"
